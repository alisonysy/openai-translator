...
class TableContent(Content):
    ...
    def set_translation(self, translation, status):
        try:
            ...
            if len(table_data[0]) and len(table_data[0]) > len(table_data[1]):
                LOG.info(f'More header columns are detected.')
                headers = []
                for h in enumerate(table_data[0]):
                    if h[0] > 0 and h[1].strip() and h[1].startswith('('):
                        h_index = len(headers)-1
                        headers[h_index] = headers[h_index] + f' {h[1]}'
                    else:
                        headers.append(h[1])
                LOG.debug(f'Reformatted headers: {headers}')
            translated_df = pd.DataFrame(table_data[1:], columns=headers)
